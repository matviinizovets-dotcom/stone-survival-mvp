...
io.on('connection', (socket) => {
  ...
  // Логи нижче тимчасові — допомагають побачити, чи взагалі доходять
  // чанки від конкретного гравця до сервера, і скільки їх та якого розміру.
  socket.on('voice-start', () => {
    console.log(`🎙️  ${shortId} почав говорити`);
  ...
  socket.on('voice-chunk', (chunk) => {
    ...
    const size = chunk && chunk.byteLength ? chunk.byteLength : (chunk ? chunk.length : 0);
    console.log(`   ↳ чанк від ${shortId}: ${size} байт`);
    // Пересилаємо як є усім іншим — без збереження, без обробки, без API ключів.
    socket.broadcast.emit('voice-chunk', { id: shortId, chunk });
  });

  socket.on('voice-end', () => {
    console.log(`🔇 ${shortId} закінчив говорити`);
